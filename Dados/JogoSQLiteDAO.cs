...
public class SQLiteJogoDAO: IJogoDAO
{
    ...
    public override void DeletarJogoPorTitulo(string titulo) {
        var transaction = connection.BeginTransaction();
        try
        {
            using (var deleteCommand = new SQLiteCommand(connection))
            {
                deleteCommand.CommandText = "DELETE FROM Jogo WHERE Titulo = @titulo";
                deleteCommand.Parameters.AddWithValue("@titulo", titulo);
